...
onion_connection_status post_check(expected_post * post, onion_request * req) {
  ...
  if (st.st_size != post->size) {
    ONION_ERROR("Size do not match, expected %d, got %d", post->size,
                st.st_size);
    post->test_ok = 0;
  }
...
void t01_post_empty_file() {
  ...
  onion_set_root_handler(server,
                         onion_handler_new((void *)&post_check, &post, NULL));
...
onion_connection_status post_json_check(json_response * post,
                                        onion_request * req,
                                        onion_response * res) {
  post->processed = 1;
  ...
  FAIL_IF_NOT_EQUAL_INT(memcmp
                        (onion_block_data(data), JSON_EXAMPLE,
                         sizeof(JSON_EXAMPLE)), 0);
...
void t05_post_content_json() {
  ...
  onion_add_listen_point(server, NULL, NULL, lp);
  onion_set_root_handler(server,
                         onion_handler_new((void *)&post_json_check, &post_json,
                                           NULL));
...
onion_connection_status post_empty_check(json_response * post,
                                         onion_request * req,
                                         onion_response * res) {
  post->processed = 1;
...
void t06_post_empty() {
  ...
  onion_add_listen_point(server, NULL, NULL, lp);
  onion_set_root_handler(server,
                         onion_handler_new((void *)&post_empty_check,
                                           &post_json, NULL));
