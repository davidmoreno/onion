	...
	while (next){
		...
		if (next->flags&OUD_STRCMP){
			...
			for (i=1;i<16;i++){
				...
				if (rm->rm_so!=-1){
					char *tmp=onion_os_scalar_malloc(rm->rm_eo-rm->rm_so+1);
...
onion_url *onion_url_new(){
	onion_url_data **priv_data=onion_os_calloc(1,sizeof(onion_url_data*));
	*priv_data=NULL;
...
int onion_url_add_handler(onion_url *url, const char *regexp, onion_handler *next){
	...
	*w=onion_os_malloc(sizeof(onion_url_data));
	...
	if (data->flags&OUD_REGEXP){
		...
		data->str=onion_os_strdup(regexp);
	}
	data->next=NULL;
	data->inside=next;
#ifdef __DEBUG__
	data->orig=onion_os_strdup(regexp);
#endif	
...
int onion_url_add_static(onion_url *url, const char *regexp, const char *text, int http_code){
	struct onion_url_static_data *d=onion_os_malloc(sizeof(struct onion_url_static_data));
	d->text=onion_os_strdup(text);
	d->code=http_code;
