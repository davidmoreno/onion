...
static onion_dict* onion_sessions_redis_get(onion_sessions* sessions, const char *session_id)
{
	...
	else
	{
		...
		{
			freeReplyObject(reply);
			reply = redisCommand(p->context, "HGET SESSIONS %s", session_id);
...
void onion_sessions_redis_save(onion_sessions *sessions, const char *session_id, onion_dict* data)
{
	...
	if(p == NULL)
	{
		redisReply* reply = redisCommand(p->context, "HDEL SESSIONS %S", session_id);
	...
	} else 
    {
		...
		redisReply* reply = redisCommand(p->context, "HSET SESSIONS %s %s", session_id, json);
