/*
	...
	Copyright (C) 2011-2012 David Moreno Montero
...
int onion_websocket_write(onion_websocket* ws, const char* buffer, size_t _len)
{
	...
	//ONION_DEBUG("Write %d bytes",len);
	{
		...
		*/
		...
		ws->req->server->write(ws->req->socket, header, hlen);
	}
	...
	for(i=0;i<len-1024;i+=1024){
		...
		ret+=ws->req->server->write(ws->req->socket, tout, sizeof(tout));
	}
	for(;i<len;i++){
	...
	return ret+ws->req->server->write(ws->req->socket, tout, len&1023);
}
...
int onion_websocket_read(onion_websocket* ws, char* buffer, size_t len)
{
	...
	int r=ws->req->server->read(ws->req->socket, buffer, len);
	if (ws->flags&WS_MASK){
		char *p=buffer;
		int i;
		for(i=0;i<r;i++){
...
static int onion_websocket_read_packet_header(onion_websocket *ws){
	...
	int r=ws->req->server->read(ws->req->socket, tmp, 2);
	...
	if (ws->flags&WS_MASK){
		r=ws->req->server->read(ws->req->socket, ws->mask, 4);
