...
int work(const char *infilename, const char *outfilename, onion_assets_file *assets){
	...
	strncpy(tmp2, tname, sizeof(tmp2)-1); // Reuse tmp2, set tname to it as it may be reused by basename
	tname=tmp2;
  ONION_DEBUG("Create init function on top, tname %s (from %s)",tname, tmp2);
...
"\n");
	...
	functions_write_main_code(&status, assets);
