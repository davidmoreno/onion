...
onion_websocket *onion_websocket_new(onion_request * req, onion_response * res) {
  ...
  const char *ws_protocol =
      onion_request_get_header(req, "Sec-Websocket-Protocol");
  const char *ws_version =
      onion_request_get_header(req, "Sec-Websocket-Version");
  ...
  if (atoi(ws_version) != 13) {
    ONION_ERROR
        ("Websocket version not supported. Currenty only support version 13, asked %s",
         ws_version);
    return NULL;
  }
...
onion_websocket_read_packet_header(onion_websocket * ws) {
  char tmp[8];
  ...
  if (r != 2) {
    ONION_DEBUG("Error reading header");
    return OCS_CLOSE_CONNECTION;
  }
  ...
  if (ws->flags & WS_MASK) {
    ...
    if (r != 4) {
      ONION_DEBUG("Error reading header (4)");
      return OCS_CLOSE_CONNECTION;
    }
    ws->mask_pos = 0;
  }
  ...
  if (ws->opcode == OWS_CONNECTION_CLOSE) {     // Closing connection
    r = (*lpreader) (ws->req, tmp, 2);
    if (r != 2) {
      ONION_DEBUG("Error reading status code");
      return OCS_CLOSE_CONNECTION;
    }
    char status[2];
    status[0] = utmp[0] ^ ws->mask[0];
    status[1] = utmp[1] ^ ws->mask[1];
    ONION_DEBUG("Connection closed by client, status=%u",
                (status[0] << 8) + status[1]);
